...
export async function renderVariant(variant, clipsDir, outputPath, onProgress) {
  ...
  // -loop 1 без явной длительности делает вход "бесконечным", что при смешивании
  // с concat может подвесить планировщик фильтров ffmpeg — поэтому каждой
  // картинке-подписи задаём конкретную длительность через -t.
  segments.forEach((seg, i) => {
    ...
    if (seg.onScreenText) {
      ...
      const pos = renderCaptionPng(pngPath, seg.onScreenText, { y: Math.round(HEIGHT * 0.78), fontSize: fontSizeFor(seg.onScreenText) });
      report(`caption seg${i} after`);
      inputArgs.push('-loop', '1', '-framerate', String(FPS), '-t', String(segDuration), '-i', pngPath);
      const capIdx = nextInputIdx++;
      filters.push(`[${label}][${capIdx}:v]overlay=${pos.x}:${pos.y}[vcap${i}]`);
      label = `vcap${i}`;
    }
  ...
  if (variant.priceCard?.show && variant.priceCard.text) {
    ...
    const pos = renderCaptionPng(pngPath, variant.priceCard.text, { y: Math.round(HEIGHT * 0.84), fontSize: 56, boxed: true });
    report('caption price after');
    inputArgs.push('-loop', '1', '-framerate', String(FPS), '-t', String(totalDuration), '-i', pngPath);
    const idx = nextInputIdx++;
    filters.push(`[${finalLabel}][${idx}:v]overlay=${pos.x}:${pos.y}[vprice]`);
    finalLabel = 'vprice';
  }
  if (variant.cta) {
    report('caption cta before');
    const pngPath = path.join(capDir, 'cta.png');
    const pos = renderCaptionPng(pngPath, variant.cta, { y: Math.round(HEIGHT * 0.92), fontSize: 54 });
    report('caption cta after');
    inputArgs.push('-loop', '1', '-framerate', String(FPS), '-t', String(totalDuration), '-i', pngPath);
    const idx = nextInputIdx++;
    filters.push(`[${finalLabel}][${idx}:v]overlay=${pos.x}:${pos.y}[vcta]`);
    finalLabel = 'vcta';
  }
  if (variant.hook) {
    report('caption hook before');
    const pngPath = path.join(capDir, 'hook.png');
    const pos = renderCaptionPng(pngPath, variant.hook, { y: Math.round(HEIGHT * 0.1), fontSize: 62 });
    report('caption hook after');
    inputArgs.push('-loop', '1', '-framerate', String(FPS), '-t', String(totalDuration), '-i', pngPath);
    const idx = nextInputIdx++;
    filters.push(`[${finalLabel}][${idx}:v]overlay=${pos.x}:${pos.y}:enable='lt(t,3)'[vhook]`);
    finalLabel = 'vhook';
  }

  const silentAudioIdx = nextInputIdx++;
  inputArgs.push('-f', 'lavfi', '-t', String(totalDuration), '-i', 'anullsrc=channel_layout=stereo:sample_rate=44100');
