...
router.post('/', requireClient, async (req, res) => {
  ...
  const { niche, audience, budget, notes, language = 'ru' } = req.body || {};
  if (!niche) return res.status(400).json({ error: 'niche обязателен' });

  const LANG_LABEL = language === 'kz' ? 'казахском' : 'русском';

  const prompt = `Подготовь короткий разбор рынка для таргетированной рекламы в Instagram/Facebook под рынок Казахстана. Пиши на ${LANG_LABEL} языке.
Ниша: ${niche}
